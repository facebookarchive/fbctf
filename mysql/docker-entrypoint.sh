...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
    ...
    if [ -z "$ADMIN_PASSWORD" ]; then
			ADMIN_PASSWORD=$(head -c 500 /dev/urandom | md5sum | cut -d" " -f1)
		fi
		HASH=$(php -f /hash.php "$ADMIN_PASSWORD")
		"${mysql[@]}" <<-EOSQL
			DELETE FROM teams WHERE name='admin' AND admin=1;
			INSERT INTO teams (id, name, password_hash, admin, protected, logo, created_ts) VALUES (1, 'admin', '$HASH', 1, 1, 'admin', NOW());
		EOSQL
		...
		echo 'The password for admin is: '$ADMIN_PASSWORD
		echo
	fi
fi
